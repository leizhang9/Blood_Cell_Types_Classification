...
def delect_temp_file(dir):
    if dir == 'prediction':
        file_list = os.listdir(os.path.join("/data","group", "model" , "inputs", dir))
        #file_list = os.listdir(BASE_DIR / 'model' / 'inputs' / dir)
        for file in file_list:
            if file[0] == '.':
                continue
            #file_path = BASE_DIR / 'model' / 'inputs' / dir / file
            file_path = os.path.join("/data","group", "model" , "inputs", dir,file)
            os.remove(file_path)
        ...
        file_list = os.listdir(os.path.join("/data","group", "model" , "inputs", "a_learning", dir))
        for file in file_list:
            if file[0] == '.':
                continue
            #file_path = BASE_DIR / 'model' / 'inputs' / 'a_learning' / dir / file
            file_path = os.path.join("/data","group", "model" , "inputs", "a_learning", dir,file)
            os.remove(file_path)