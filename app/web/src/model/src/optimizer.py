...
class AOptimizer(DataLoader):
    ...
    def _epoch_checkpoint_save(self, epoch: int, file_name: str) -> None:
        ...
        if not os.path.exists(os.path.join("/data","group", "model", "outputs" , "checkpoints")):
            os.makedirs(os.path.join("/data","group", "model" , "outputs" , "checkpoints"))

        mode = load_settings()['mode']
        torch.save(self.model.state_dict(),
                   os.path.join("/data","group", "model" , "outputs" , "checkpoints", f"checkpoint_{mode}.pth"))
    ...
    def _sort_uncertainties(self, pred_output: dict):
        ...
        file_path = os.path.join("/data","group", "model", "inputs" ,Usage.A_LEARNING , "temp" , "pr_dataset.npy")
